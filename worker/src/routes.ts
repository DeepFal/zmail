...
function parseExpiresInHours(value: unknown): number {
  ...
  if (typeof value !== 'number' || !Number.isFinite(value) || !Number.isInteger(value)) {
    throw new Error('expiresInHours 必须是整数');
  }

  if (value < 1 || value > 168) {
    throw new Error('expiresInHours 必须在 1 到 168 小时之间');
  }
...
app.post('/api/mailboxes', async (c) => {
  try {
    const body = await c.req.json() as { address?: string; domain?: string; expiresInHours?: unknown };
